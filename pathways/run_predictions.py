...
def main():
    try:
        ...
        if not assoc_proteins_set:
            raise ValueError(f"No valid proteins after cleaning for: {disease_name}")
        ...
        if np.sum(assoc_gene_vector) == 0:
            raise ValueError("No matching proteins between disease and network")
        ...
        print("\n=== Evaluation Metrics ===")
        true_indices = set(i for i, node in enumerate(nodes) 
                          if format_protein_id(node) in assoc_proteins_set)
        metrics = {}
